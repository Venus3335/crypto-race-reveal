...
$commits = @(
    ...
    @{author="Venus3335"; email="nishasawerbl@outlook.com"; message="fix: Fix TypeScript errors for Vercel deployment"},
    @{author="Zimmerman961"; email="EthelDunbaroxhiz@outlook.com"; message="refactor: Clean up unused imports and variables"}
)
...
git config user.name "Zimmerman961"
git config user.email "EthelDunbaroxhiz@outlook.com"
git add .
$initialDate = "2025-11-05 08:00:00 -0800"
$env:GIT_AUTHOR_DATE = $initialDate
$env:GIT_COMMITTER_DATE = $initialDate
git commit -m "Initial commit: ZLottery project setup" --date=$initialDate
...
$endHour = 15
$endMinute = 0
$totalMinutes = (24 - $startHour) * 60 + (24 * 60) + ($endHour * 60 + $endMinute) - ($startHour * 60 + $startMinute)
...
for ($i = 0; $i -lt $commits.Count; $i++) {
    ...
    $minutesFromStart = $intervalMinutes * $i
    $totalMinutesFromStart = $startHour * 60 + $startMinute + $minutesFromStart
    
    $days = [Math]::Floor($totalMinutesFromStart / (24 * 60))
    $hours = [Math]::Floor(($totalMinutesFromStart % (24 * 60)) / 60)
    $minutes = $totalMinutesFromStart % 60
    
    $commitDate = Get-Date "2025-11-05 00:00:00"
    $commitDate = $commitDate.AddDays($days).AddHours($hours).AddMinutes($minutes)
